...
export default {
  ...
  async run({ sock, msg, from, sender }) {
    ...
    try {
      const apiRes = await fetch('https://api.otakugifs.xyz/gif?reaction=hug')
      ...
      const id = Date.now()
      const gifPath = path.join(os.tmpdir(), `hug-${id}.gif`)
      const mp4Path = path.join(os.tmpdir(), `hug-${id}.mp4`)
      ...
      const caption = target === sender
        ? `🤗 @${senderNumber} se abrazó a sí mismo`
        : `🤗 @${senderNumber} le dio un abrazo a @${targetNumber}`
